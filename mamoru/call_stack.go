...
var _ vm.Tracer = (*CallStackTracer)(nil)
...
func (t *CallStackTracer) CaptureEnd(output []byte, gasUsed uint64, time time.Duration, err error) {
	t.Lock()
	defer t.Unlock()
	txIndex := t.txIndex.Load()
...
func (t *CallStackTracer) CaptureState(env *vm.EVM, pc uint64, op vm.OpCode, gas uint64, cost uint64, scope *vm.ScopeContext, rData []byte, depth int, err error) {
...
func (t *CallStackTracer) CaptureFault(env *vm.EVM, pc uint64, op vm.OpCode, gas uint64, cost uint64, scope *vm.ScopeContext, depth int, err error) {
	// Skip if tracing was interrupted
	if atomic.LoadUint32(&t.interrupt) > 0 {
		return
	}
